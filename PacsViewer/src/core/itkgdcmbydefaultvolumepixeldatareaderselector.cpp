...
VolumePixelDataReaderFactory::PixelDataReaderType ItkGdcmByDefaultVolumePixelDataReaderSelector::selectVolumePixelDataReader(Volume *volume) const
{
    ...
    foreach (Image *image, imageSet)
    {
        ...
        /// TODO This check could in theory be removed because images of different
        /// size are separated in different volumes, but we are keeping it just in case
        ...
        {
            /// If photometric interpretation is not MONOCHROME1 nor MONOCHROME2,
            /// then it's a color image: PALETTE COLOR, RGB, YBR_FULL, YBR_FULL_422,
        ...
        else if (image->getBitsAllocated() == 16 && image->getBitsStored() == 16 && !image->getSOPInstanceUID().contains("MHDImage"))
        {
            ...
            /// itk::Image has a fixed pixel type, currently signed short,
            /// but with 16 bits allocated and 16 bits stored the needed type might be unsigned short
            /// in order for the image to be correctly displayed

            /// Apply only for "non-volumetric" image modalities to narrow
            /// this problem and avoid the problem described in ticket #1313
