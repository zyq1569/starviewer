...
/**
     ...
     -accessionnumber valueAccessionNumber: Search the study with the accession value specified in the PACS marked to search by
                                             defect and if it finds it download it.
     (blank): Running a starviewer instance without any parameters opens starviewer with a blank window.
   */
...
public:
    ...
    /// indicate that there are requests via line order to be ateges
    bool parseAndRun(QStringList arguments, QString &errorInvalidCommanLineArguments);

    /// Parse a list of arguments passed in a QString separated by ';', returns boolea indicating whether the arugments in the list
