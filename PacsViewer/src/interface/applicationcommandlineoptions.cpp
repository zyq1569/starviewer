    ...
    while (!argumentList.isEmpty() && m_parserErrorMessage.isEmpty())
    {
        ...
        if (isAnOption(parameter))
        {
            if (!nextParameterHasToBeAnArgumentOption)
            {
                ...
                {
                    // If it is an option that we have specified as valid,
                    // we insert it as a parsed, for now as an argument of the option we put ""
        ...
        else
        {
            ...
            if (lastParameterWasAnOption)
            {
                if (nextParameterHasToBeAnArgumentOption)
                {
                    // If we have an argument and the last parameter was an option, it means that parameter is an argument
